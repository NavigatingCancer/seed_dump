...
module SeedDump
  class Perform
    ...
    def setup(env)
      ...
      @opts['file']    = env['FILE'] || "#{Rails.root}/db/seed_dump.rb"
    ...
    def loadModels
      ...
      Dir[Dir.pwd + '/' + @opts['model_dir']].sort.each do |f|
        ...
        begin
          ...
          puts "Skipping #{model} due to #{e.class.name}"
          next
        end

        next if model == 'AllCancerType'
        next if model == 'Clinic::Visit'
        next if model == 'Clinic::MeaninfulUseReport'
    ...
    def dumpModels
      FileUtils.rm_rf Rails.root.join("db/seed_dumps")
      @models.sort.each do |model|
        filename = Rails.root.join("db/seed_dumps/#{model.underscore}.rb")
        FileUtils.mkdir_p(File.dirname(filename))
        File.open(filename, 'w') do |f|
          if @habtms.include? model
            define_class = "class ::#{model} < ActiveRecord::Base; self.table_name = '#{model.underscore}'; end\n"
            f.puts define_class
            eval define_class
          end 

          m = model.constantize
          if m.ancestors.include?(ActiveRecord::Base) && !m.abstract_class && m.connection.table_exists?(m.table_name)
            puts "Adding #{model} seeds." if @opts['verbose']

            if @opts['skip_callbacks']
              f.puts "#{model}.reset_callbacks :validation"
              f.puts "#{model}.reset_callbacks :validate"
              f.puts "#{model}.reset_callbacks :save"
              f.puts "#{model}.reset_callbacks :create"
    ...
    def writeFile
      File.open(@opts['file'], (@opts['append'] ? "a" : "w")) { |f|
        ...
        f.puts "ActiveRecord::Base.connection.execute('SET FOREIGN_KEY_CHECKS=0')"
        @models.sort.each do |model|
          f.puts "require File.expand_path('../seed_dumps/#{model.underscore}', __FILE__)"
        end
        f.puts "ActiveRecord::Base.connection.execute('SET FOREIGN_KEY_CHECKS=1')"
      }
    end
    ...
    def attribute_for_inspect(r,k)
      ...
      avoid_weekends = true
      if %w{Event TreatmentEvent TrackerSignallValue VisitPreparation}.include?(r.class.name)
